...
;; Smoke test for examples/framed-echo.rkt: loading the module runs its
;; `main`, which drives a loopback TCP framed echo and a UDP request/reply
;; and prints the results.  We capture stdout and check the stable lines
;; (the server's "client from …:PORT" line carries an ephemeral port, so
;; it is matched loosely).
...
(define out
  (parameterize ([current-output-port (open-output-string)])
    (dynamic-require framed-echo-example #f)

...
(test-case "non-parametric record derives Eq/Show/Ord"
  (check-true  same-pt)
  (check-false diff-pt)
  (check-true  lt-pt)
  (check-equal? shown-pt "(Point 7 9)"))

(test-case "parametric record derives Functor + Foldable"
  (check-equal? mapped-box (Box 42))
  (check-equal? summed-box 100)
  (check-equal? listed-box (Cons 5 Nil)))

(test-case "parametric ADT Eq + Show with constraint propagation"
  (check-true   pair2-eq)
  (check-equal? pair2-show "(MkPair2 7 \"x\")"))

(test-case "recursive ADT Foldable"
  (check-equal? tree-sum  10)

...
;;
;; The example's `main` runs via its auto-generated `module+ main`,
;; printing one inferred type per demo term.  We instantiate that
;; submodule for effect, capture stdout, and pin every reported type
;; and type-error so a
...
(define output
  (squeeze
    (with-output-to-string
      (lambda () (dynamic-require `(submod ,thih-example main) #f)))))

...
(test-case "membership / size"
  (check-equal? mem1 #t) (check-equal? mem3 #f)
  (check-equal? emp0 #t) (check-equal? emp1 #f)
  (check-equal? sz 2) (check-equal? tolen 2))

(test-case "singleton / find-with-default"
  (check-equal? single (Some "z"))
  (check-equal? fwd-hit "b") (check-equal? fwd-miss "d"))

(test-case "adjust / insert-with"
  (check-equal? adj (Some "a!"))
  (check-equal? iw  (Some "xa")))

(test-case "union / union-with"
  (check-equal? uni  (Some "L"))
  (check-equal? uniw (Some "LR")))

(test-case "difference / intersection"
  (check-equal? diff1 #f) (check-equal? diff2 #t)
  (check-equal? isect (Some 220)))

(test-case "map / filter"
  (check-equal? mm  (Some 20))

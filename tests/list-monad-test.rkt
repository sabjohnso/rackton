...
(test-case "flatmap over List is concatMap"
  (check-equal? dupped (Cons 1 (Cons 1 (Cons 2 (Cons 2 (Cons 3 (Cons 3 Nil))))))))

(test-case "join flattens one level of nesting"
  (check-equal? flattened (Cons 1 (Cons 2 (Cons 3 (Cons 4 (Cons 5 Nil)))))))

(test-case "do over List is the cartesian product"
  (check-equal? pairs
                (Cons (MkPair 1 10)
                      (Cons (MkPair 1 20)
                            (Cons (MkPair 2 10)
...
(test-case "flatmap to Nil drops that element"

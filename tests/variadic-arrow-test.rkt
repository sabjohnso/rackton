...
(test-case "variadic -> in signature: ternary fn type-checks and runs"
  (check-equal? (add3 1 2 3) 6))

(test-case "variadic -> in signature: binary fn type-checks and runs"
  (check-equal? (const2 5 7) 5))

(test-case "variadic -> mixed with higher-order arg"
  (check-equal? (apply-twice (+ 1) 10) 12))

(test-case "partial application across a variadic-form signature"
  (check-equal? ((add3-curried 20) 30) 60))

(test-case "variadic -> in a class method signature"
  (check-equal? tri-int 6))

(test-case "1-arg `(-> T)` (0-arg fn encoding) still works"

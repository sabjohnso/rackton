...
(test-case "Rational arithmetic"
  (check-equal? five-sixths (make-rational 5 6))
  (check-equal? one-sixth   (make-rational 1 6))
  (check-equal? half-shown  "1/2"))

(test-case "Complex arithmetic"
  (check-equal? c-sum  (make-complex  4.0  6.0))
  (check-equal? c-prod (make-complex -5.0 10.0))
  (check-true   (< (abs-float (- c-mag 5.0)) 0.000001)))

(test-case "Integral class on Integer"
  (check-equal? int-div-result   3)
  (check-equal? int-mod-result   2)
  (check-equal? int-quot-result  3)
  (check-equal? int-rem-result   2))

(test-case "Real class to-rational"
  (check-equal? from-int (make-rational 7 1))
  (check-equal? from-rat half))

(test-case "Floating class basics"
...
(test-case "RealFrac floor/round/ceiling/truncate"
  (check-equal? floored   3)
  (check-equal? rounded   4)  ;; banker's or away-from-zero — accept Racket default
  (check-equal? ceiled    4)
  (check-equal? truncated -3))

(test-case "RealFloat is-nan? / is-infinite?"
  (check-true  nan-is-nan)

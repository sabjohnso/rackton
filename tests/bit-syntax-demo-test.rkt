...
;; Smoke test for examples/bit-syntax.rkt: loading the module runs its
;; `main`, which builds and parses a sub-byte header and round-trips a
;; length-prefixed frame.  We capture stdout and check the stable lines.
...
(define out
  (parameterize ([current-output-port (open-output-string)])
    (dynamic-require bit-syntax-example #f)

...
(test-case "single-threaded TVar mutation"
  (check-equal? (run-io simple-mutate) 42))

(test-case "concurrent counter: 2 threads × 100 increments = 200"
  (check-equal? (run-io concurrent-counter) 200))

(test-case "or-else falls back on retry"
  (check-equal? (run-io orelse-result) 7))

(test-case "do-notation chain over STM"

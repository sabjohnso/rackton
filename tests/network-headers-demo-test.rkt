...
;; Smoke test for examples/network-headers.rkt: instantiating its
;; `module+ main` runs `main`, which builds a UDP-over-IPv4 datagram
;; with bit syntax and parses it back.  We capture stdout and check
;; the stable lines.
...
(define out
  (parameterize ([current-output-port (open-output-string)])
    (dynamic-require `(submod ,network-headers-example main) #f)
